...
<head>
    ...
    <title>Change username({{ Auth::user()->name }})</title>
...
<body class="bg-green-300 flex justify-center">
    ...
    <div class="bg-green-400 absolute top-36 flex justify-center flex-co w-1/3 rounded-2xl px-10">
        <form class="flex flex-col p-10 w-full" action="{{ route('phonebook.user.profile.username' , ['id'=>Auth::user()->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <label for="name" class="block text-black text-xl font-medium ml-1">Enter new username</label>
            <input class="w-full h-11 text-xl border-green-600 rounded-lg 
            shadow-sm ml-1 mb-5" type="text" name="name">
